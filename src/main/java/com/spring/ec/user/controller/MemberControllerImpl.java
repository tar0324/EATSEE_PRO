...
public class MemberControllerImpl implements MemberController {
	...
	public ModelAndView find_id_Result(@ModelAttribute("findidmem") MemberVO member,RedirectAttributes rAttr, HttpServletRequest request,HttpServletResponse response) throws Exception {
		String viewName = (String) request.getAttribute("viewName");
		ModelAndView mav = new ModelAndView();
		String user_id = memService.find_id_Result(member);
		if(user_id == null) {  //회원정보를 찾을 수 없다면
	...
	public ModelAndView find_pwd_Result(@ModelAttribute("findpwdmem") MemberVO memberVO,HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		/*
		 * String user_name = request.getParameter("user_name"); String user_id =
		 * request.getParameter("user_id"); String user_email =
		 * request.getParameter("user_email"); memberVO.setUser_name(user_name);
		 * memberVO.setUser_id(user_id); memberVO.setUser_email(user_email);
		 */
	...
	// 로그인 배너 이동
	@Override
	@RequestMapping(value = "/login_banner", method = RequestMethod.GET)
	...
	// 비회원 예약 조회
	@Override
	@RequestMapping(value = "/nouserbook", method = RequestMethod.GET)
	public ModelAndView nouserbook(@RequestParam(value = "user_name") String user_name,@RequestParam(value = "order_id") String order_id,RedirectAttributes rAttr, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		listMap.put("user_name", user_name);
		listMap.put("order_id", order_id);
		NoUserBookVO nouser = memService.selectnouser(listMap);
		
		if (nouser != null) { /* DB에 있다면 (예약 건이 있다면)*/
			rAttr.addFlashAttribute("nouser", nouser);
			rAttr.addFlashAttribute("result", "nousercheck");
			mav.setViewName("redirect:/user/loginForm.do");
			
			/* mav.setViewName(viewName); */
		} else {
			rAttr.addFlashAttribute("result", "nouserFailed"); /* login.jsp의 loginFailed <choose>구문 실행 */
			mav.setViewName("redirect:/user/loginForm.do"); /* login.jsp redirect한다 */
		}
	...
	//비회원 예약 조회 확인창
	@Override
	@RequestMapping(value = "/nousercheckbook", method = RequestMethod.GET)
